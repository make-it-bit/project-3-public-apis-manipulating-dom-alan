...
async function getCryptoDataJson() {
  ...
  for (let i = 0; i < checkboxContainers.length; i++) {
    let isCheckboxChecked = checkboxContainers[i].getElementsByClassName('check-input')[0].checked;

    if (isCheckboxChecked) {
      let checkboxValue = checkboxContainers[i].getElementsByTagName('span')[0].innerHTML;
      let checkBoxId = checkboxContainers[i].getElementsByClassName('check-input')[0].id;
      output += `<li>${checkboxValue}: ${Number(jsonData[checkBoxId]).toFixed(4)}</li>`;
    }
...
function addLoader() {
  ...
  document.getElementById('crypto-data-container').innerHTML += '<div class="loader"></div>';
}

function removeLoader() {
  document.getElementById('crypto-data-container').removeChild(document.querySelector('.loader'));
}

function showCryptoDataContainer(cryptoData) {
  document.getElementById('crypto-data-container').style.display = 'block';
  document.getElementById('crypto-data-container-title').innerHTML = `Data for ${jsonData['symbol']}`;
